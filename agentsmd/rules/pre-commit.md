...
| --- | --- | --- |
| Pre-commit hook definitions (Nix consumers) | [`amatos/nix-devenv`](https://github.com/amatos/nix-devenv) — `lib/pre-commit-hooks.nix` + `flake-modules/profiles/<name>.nix` | `imports = [ inputs.nix-devenv.flakeModules.<profile> ]` in the consumer's `flake.nix` |
| Pre-commit hook definitions (non-Nix consumers) | [`amatos/.github`](https://github.com/amatos/.github) `precommit/templates/<profile>.yaml` | Copy at scaffold; Renovate keeps `rev:` pins fresh |
| Shared lint configs (`.markdownlint-cli2.yaml`, `.tflint.hcl`, `.ansible-lint`, `.yamllint.yml`) | `amatos/.github` — `.markdownlint-cli2.yaml` at root for backward compat; the rest in `precommit/configs/` | Nix path: `nix-devenv.lib.fetch-shared-configs` materializes nix-store paths; non-Nix path: copy at scaffold |
| `zizmor.yml` workflow-security policy | `amatos/.github` at the root | Same as above — passed as `--config` by the base profile's zizmor hook |
...
gh api repos/amatos/.github/contents/precommit/templates/terraform.yaml \
  -H "Accept: application/vnd.github.raw" > .pre-commit-config.yaml

# Materialize the configs the hooks need
gh api repos/amatos/.github/contents/precommit/configs/tflint.hcl \
  -H "Accept: application/vnd.github.raw" > .tflint.hcl
gh api repos/amatos/.github/contents/.markdownlint-cli2.yaml \
  -H "Accept: application/vnd.github.raw" > .markdownlint-cli2.yaml
gh api repos/amatos/.github/contents/zizmor.yml \
...
- Do NOT duplicate shared lint config files (`.markdownlint`,
  ...
  `amatos/.github` at scaffold (non-Nix path).
...
- See [`precommit/README.md` in `amatos/.github`](https://github.com/amatos/.github/blob/main/precommit/README.md)
